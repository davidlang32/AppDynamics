...
# Controller Settings
#----------------------------------------------------------------------------------------------
controllerhost='deltekprod.saas.appdynamics.com'
controllerport='443'
controllerssl='true'
orchestration='false'
uniquehostid=$host_name
accountaccesskey='sbjnk0kcshlh'
accountname='deltekprod'
sim='true'
issapmachine=''
machinepath=''
...
if [ -d /etc/systemd/system ] && [ -d /etc/init.d ]; then
  echo "Both /etc/systemd/system and /etc/init.d exist. Updating /etc/systemd/system"
  ...
  echo "/etc/systemd/system exists. Updating /etc/systemd/system"
  sed -i "s/User=appdynamics-machine-agent/User=$machine_agent_user/" /opt/appdynamics/machine-agent/etc/systemd/system/appdynamics-machine-agent.service
  sed -i "s/Environment=MACHINE_AGENT_USER=appdynamics-machine-agent/Environment=MACHINE_AGENT_USER=$machine_agent_user/" /opt/appdynamics/machine-agent/etc/systemd/system/appdynamics-machine-agent.service
  cp /opt/appdynamics/machine-agent/etc/systemd/system/appdynamics-machine-agent.service /etc/systemd/system -f
  systemctl daemon-reload
  ...
  echo "/etc/init.d/ exists. Updating machine-agent/etc/init.d"
  cp /opt/appdynamics/machine-agent/etc/init.d/appdynamics-machine-agent /etc/init.d -f
  systemctl daemon-reload
  systemctl enable appdynamics-machine-agent
  systemctl start appdynamics-machine-agent
else
  echo "Neither /etc/systemd/system nor /etc/init.d exist. Exiting..."
  exit 1
fi
echo " ______________________________________________________________________________________________________________________"
echo "|                                                                                                                      |"
